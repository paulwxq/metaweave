...
COMMENT ON COLUMN public.bss_service_area.created_by IS '创建操作的用户账号';
COMMENT ON COLUMN public.bss_service_area.update_ts IS '记录最后更新时间戳';
COMMENT ON COLUMN public.bss_service_area.updated_by IS '最后更新操作的用户账号';
COMMENT ON COLUMN public.bss_service_area.delete_ts IS '逻辑删除时间戳（为空表示未删除）';
COMMENT ON COLUMN public.bss_service_area.deleted_by IS '执行逻辑删除的用户账号';
