...
COMMENT ON COLUMN public.qa_feedback.sql IS '生成的对应SQL查询语句';
COMMENT ON COLUMN public.qa_feedback.is_thumb_up IS '用户是否点赞（True-点赞，False-未点赞）';
COMMENT ON COLUMN public.qa_feedback.user_id IS '提交反馈的用户ID';
COMMENT ON COLUMN public.qa_feedback.create_time IS '反馈创建时间戳';
COMMENT ON COLUMN public.qa_feedback.is_in_training_data IS '是否已纳入训练数据集（True-是，False-否）';
COMMENT ON COLUMN public.qa_feedback.update_time IS '反馈最后更新时间戳';
...
-- Table Comment
COMMENT ON TABLE public.qa_feedback IS '用户对SQL查询结果的反馈记录，包括问题、生成SQL、点赞状态及是否用于训练';
