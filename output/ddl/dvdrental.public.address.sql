...
COMMENT ON COLUMN public.address.address2 IS '额外地址信息（可选）';
COMMENT ON COLUMN public.address.district IS '所属行政区或地区名称';
COMMENT ON COLUMN public.address.city_id IS '所属城市唯一标识ID';
COMMENT ON COLUMN public.address.postal_code IS '邮政编码';
COMMENT ON COLUMN public.address.phone IS '联系电话号码';
COMMENT ON COLUMN public.address.last_update IS '最后更新时间戳';

-- Indexes
CREATE INDEX idx_fk_city_id ON public.address(city_id);

-- Table Comment
COMMENT ON TABLE public.address IS '地址信息表，存储客户的详细地址及联系方式';
