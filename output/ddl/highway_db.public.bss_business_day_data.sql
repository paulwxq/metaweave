...
COMMENT ON COLUMN public.bss_business_day_data.delete_ts IS '逻辑删除时间戳（为空表示未删除）';
...
COMMENT ON COLUMN public.bss_business_day_data.xs IS '信用卡支付金额（单位：元）';
...
COMMENT ON COLUMN public.bss_business_day_data.pay_sum IS '当日总支付金额（单位：元）';
...
-- Table Comment
COMMENT ON TABLE public.bss_business_day_data IS '服务区各营业网点按日统计的多渠道（微信、支付宝、现金等）交易金额与订单数明细';
