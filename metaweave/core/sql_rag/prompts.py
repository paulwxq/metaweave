...
7. 当问题要求统计某类业务实体的数量（如服务区数量、公司数量、路线数量、用户数量）时，应优先在该实体对应的维度表上计数；如果必须通过桥接表或明细表关联，必须对该实体主键使用 COUNT(DISTINCT ...) 去重，不要直接把桥接表行数或关联行数当作实体数量。
8. SQL 必须可执行，避免引用不存在的字段、表、别名或聚合错误。
9. 每条 SQL 必须是单行文本，并且以分号结尾。
10. question 与 SQL 必须直接对应，SQL 的执行结果应能回答问题。
...
4. JOIN 条件需符合上述表间关系
5. 如果原问题是在统计某类业务实体的数量（如服务区数量、公司数量、路线数量、用户数量），应优先在该实体对应的维度表上计数；如果必须通过桥接表或明细表关联，必须对该实体主键使用 COUNT(DISTINCT ...) 去重，不要直接把桥接表行数或关联行数当作实体数量。
6. 所有 SQL 必须以分号结尾
7. 返回严格的 JSON 数组
