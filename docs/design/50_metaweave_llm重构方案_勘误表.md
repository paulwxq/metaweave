...
1. **错误的模块路径**：`python -m metaweave.cli.metadata_cli`
2. **错误的选项名**：`--table`（单数）
...
```bash
# ❌ 错误命令
python -m metaweave.cli.metadata_cli \
...
# metadata 子命令帮助
python -m metaweave.cli.main metadata --help
```
...
# 5. 启用调试模式
python -m metaweave.cli.main --debug metadata \
    --config configs/metaweave/metadata_config.yaml \
    --step json_llm \
    --tables public.dim_product
```
...
- [x] 确认 CLI 入口是 `metaweave.cli.main`
- [x] 确认选项是 `--tables`（复数）
