...
def __init__(self, config: Dict[str, Any] = None):
    ...
    # - 配置缺失时工厂函数会抛出明确的异常
    from services.vector_adapter import create_vector_search_adapter
...
from services.config_loader import load_subgraph_config
...
```python
from services.db.pg_client import get_pg_client
from services.db.neo4j_client import get_neo4j_client
from services.vector_adapter import create_vector_search_adapter  # 新增
...
1. **先完成 MilvusClient 下沉（同一 PR 内完成，避免中间态不可运行）**：
   ...
   - **保留旧路径兼容**：在 `src/metaweave/services/vector_db/milvus_client.py` 改为 re-export：
     ...
     from services.vector_db.milvus_client import (  # noqa: F401
         MilvusClient,
         _lazy_import_milvus,
     )
     ```
...
from services.db.pg_client import get_pg_client
from services.vector_adapter.base import BaseVectorSearchAdapter
...
from services.vector_db.milvus_client import MilvusClient, _lazy_import_milvus
from services.vector_adapter.base import BaseVectorSearchAdapter
...
class MilvusSearchAdapter(BaseVectorSearchAdapter):
    ...
    def _validate_embedding_dimension(self):
        ...
        """
        from services.embedding.embedding_client import get_embedding_client
...
from services.vector_adapter.base import BaseVectorSearchAdapter
from services.vector_adapter.pgvector_adapter import PgVectorSearchAdapter
from services.vector_adapter.milvus_adapter import MilvusSearchAdapter
