...
```bash
# LLM 辅助流程（推荐）
python -m metaweave.cli.main metadata --config configs/metaweave/metadata_config.yaml --step ddl
...
python -m metaweave.cli.main metadata --config configs/metaweave/metadata_config.yaml --step cql_llm
python -m metaweave.cli.main load --type cql --clean

# 传统流程（命令行用法不变，CQL 方向逻辑已修复）
python -m metaweave.cli.main metadata --config configs/metaweave/metadata_config.yaml --step ddl
python -m metaweave.cli.main metadata --config configs/metaweave/metadata_config.yaml --step json
python -m metaweave.cli.main metadata --config configs/metaweave/metadata_config.yaml --step rel
python -m metaweave.cli.main metadata --config configs/metaweave/metadata_config.yaml --step cql
python -m metaweave.cli.main load --type cql --clean
```
...
from metaweave.utils.data_utils import get_column_statistics, dataframe_to_sample_dict
