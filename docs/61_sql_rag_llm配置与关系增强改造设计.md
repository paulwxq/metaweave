...
2. 以 `### ` 为段落边界
3. 增强匹配：复用原有的点模式机制（`.table.`），并将其升级为包含 schema 的精确前缀（`schema.table.`），而不是只用纯表名。**匹配判定仅应发生在每一段的标题行，绝对不可扫描大段正文，以防把正文提及时的话语误认**。

> **已知限制**：原版（`_extract_relevant_relationships`）虽存在点模式，但由于仅按纯表名匹配，对多 schema / 同名表环境依然不稳，容易误召回。
> 正式实施的共享函数需升级为更准确的"多级别回退匹配"。
...
- 输入优先允许传入 `db.schema.table` 或 `schema.table` 格式的全限定名。
...
- **特殊说明**：若上游（如 `db_domains.yaml`）仅配置了纯表名（如 `orders`），传入共享函数后将由于缺失 schema 而无法进行 `schema.table.` 匹配，最终自然退化到纯表名兜底。此为系统符合预期的表现；如需确保多 schema 下的精确关联提取，必须要求在上游配置阶段书写带 schema 的全限定名。
...
> **实现坑点提示**：以上为最终展示的 JSON 文本范例。但在落地 `prompts.py` 时，请务必注意它将被嵌套在 `str.format()` 的模板字符串内。因此模板里的 JSON 大括号必须转义写作双重格式 `{{` 及 `}}`，否则运行时会触发格式化 KeyError！
...
- `[待实施]` 将 `rel_directory` 传给生成器。推导获取路径为：`rel_dir = main_config.get("output", {}).get("rel_directory", "output/rel")`，可不动 CLI 参数。
...
- `[待实施]` 在 `_step_sql_rag_generate()` 中同理推导路径 `rel_dir = ctx.loaded_config.get("output", {}).get("rel_directory", "output/rel")`，并将其传给生成器。

### 8.6 `[待实施]` `metaweave/core/sql_rag/context_utils.py`（新建）

将 `SQLValidator._extract_relevant_relationships()` 的逻辑抽取为共享函数，供生成器和验证器共同使用。
> **重构强制准则（实施方案 A）**：彻底删除验证器中原有的私有方法 `_extract_relevant_relationships`，将其所有调用点全部替换为新建好的共享模块函数，阻断两套独立抽取规则共存的可能。
...
1. **读取已在 YAML 示例（Section 5.1）中声明的 `sql_rag.generation.max_prompt_chars` 配置**（如 100000 字符）。但在 `generator.py` 层级，该参数实际的读取与校验代码仍为 `[待实施]` 状态。
2. **仅告警不截断**：基于最完整的 MD 和 Rel 结构直接构建提示词，如果系统发现总字符数超限，仅仅在日志中打印一条具有警示意义的 `logger.warning("主题域 xxx 生成提示词长度超限（{} > {}），可能遭遇报错或截断")`。不对 Prompt 进行人为“缩水”处理。
3. **透传至大模型**：无论超限与否，全部原样请求大模型，由模型 API 或提供方决断是否直接返回 `token exceeded limit` 错误。如有错误，则正常走当前框架默认的失败跳过流程。
...
3. 生成器层与 Prompt 层（紧密耦合，须同步修改防 KeyError）：
   给 `QuestionSQLGenerator` 增加 `rel_dir` 支持，在请求构造前调用共享提取；同时升级 prompt，引入 `{rel_content}` 和对应占位文案。两端不可拆分单独提交。
4. ~~CLI / Pipeline 层：接入 `sql_rag.llm` 配置覆盖~~ `[已完成]`
   剩余：在 CLI/Pipeline 中将 `rel_dir` 传给生成器
5. Prompt 预算控制：
   在 `generator.py` 中实现 prompt 长度统计与超限告警（不做自动裁剪）
6. 测试层：
